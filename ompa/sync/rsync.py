...
class RsyncBackend(SyncBackend):
    ...
    def push(self, vault_path: Path, message: str = "") -> SyncResult:
        ...
        transferred = len([line for line in stdout.splitlines() if line and not line.startswith("sending")])
    ...
    def pull(self, vault_path: Path) -> SyncResult:
        ...
        transferred = len([line for line in stdout.splitlines() if line and not line.startswith("receiving")])
        return SyncResult(
            success=True,
            backend=self.name,
            direction="pull",
    ...
    def status(self, vault_path: Path) -> SyncResult:
        ...
        would_transfer = [line for line in stdout.splitlines() if line and not line.startswith(("sending", "sent", "total"))]
        return SyncResult(
            success=True,
            backend=self.name,
            direction="status",
