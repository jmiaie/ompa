...
def _is_excluded(path: Path, exclude_patterns: list[str]) -> bool:
    """Return True if *path* matches any of the exclusion patterns."""
    path_str = str(path)
...
class Vault:
    ...
    def _resolve_brain_note_path(self, name: str) -> Path:
        """
        Resolve and validate a brain note name, returning the absolute path.
        Raises ValueError if the name contains path traversal components.
        """
