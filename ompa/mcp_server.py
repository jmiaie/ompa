...
def _dispatch_tool(name: str, arguments: dict, vault_path: str) -> dict:
    """Route a validated tool call to the appropriate implementation."""
    # Tools that only need vault_path — no other arguments
    _SIMPLE = {
    ...
    # Tools that receive the full arguments dict (manage vault selection themselves)
    _PASSTHROUGH = {
        "ao_write":  ao_write,
        "ao_export": ao_export,
        "ao_import": ao_import,
    ...
    if name in _PASSTHROUGH:
        return _PASSTHROUGH[name](arguments)

    # Tools with explicit required/optional parameters
    if name == "ao_classify":
