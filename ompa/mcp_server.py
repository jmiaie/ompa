...
def _dispatch_tool(name: str, vault_path: str, arguments: dict) -> dict:
    ...
    Tools that accept only vault_path are handled via a simple lookup table.
    Tools that require additional arguments from the request are handled
    individually below.
    """
    # Simple tools: callable(vault_path) → result
    _simple = {
    ...
    if name in _simple:
        return _simple[name](vault_path)

    # Tools that pass the full arguments dict (handle dual-vault paths internally)
    if name == "ao_write":
    ...
    # Tools that need specific arguments extracted from the request
    if name == "ao_classify":
