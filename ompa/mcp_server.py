...
def _make_ompa_from_args(arguments: dict, enable_semantic: bool = False):
    """Extract vault-related keys from an arguments dict and return an Ompa instance."""
    vault_kwargs = {k: arguments[k] for k in _VAULT_ARG_KEYS if k in arguments}
    return make_ompa(**vault_kwargs, enable_semantic=enable_semantic)


def ao_write(arguments: dict) -> dict:
    """Write content to the appropriate vault (auto-classifies in dual mode)."""
    ao = _make_ompa_from_args(arguments)
...
def _dispatch_tool(name: str, arguments: dict, vault_path: str) -> dict:
    """Dispatch a validated tool call to its handler. Raises KeyError on missing args."""
    # Tools that receive the raw arguments dict (handle vault resolution internally)
    if name in ("ao_write", "ao_export", "ao_import"):
        handlers = {"ao_write": ao_write, "ao_export": ao_export, "ao_import": ao_import}
        return handlers[name](arguments)

    # Tools that only need vault_path
    simple = {
        "ao_session_start": lambda: ao_session_start(vault_path),
        "ao_kg_stats":      lambda: ao_kg_stats(vault_path),
        "ao_palace_wings":  lambda: ao_palace_wings(vault_path),
        "ao_wrap_up":       lambda: ao_wrap_up(vault_path),
        "ao_status":        lambda: ao_status(vault_path),
        "ao_orphans":       lambda: ao_orphans(vault_path),
        "ao_kg_populate":   lambda: ao_kg_populate(vault_path),
        "ao_sync":          lambda: ao_sync(vault_path),
        "ao_init":          lambda: ao_init(vault_path),
    }
    if name in simple:
        return simple[name]()

    # Tools that need additional arguments
    if name == "ao_classify":
        return ao_classify(message=arguments["message"], vault_path=vault_path)
    if name == "ao_search":
        return ao_search(query=arguments["query"], vault_path=vault_path,
                         limit=arguments.get("limit", 5))
    if name == "ao_kg_query":
        return ao_kg_query(entity=arguments["entity"], vault_path=vault_path,
                           as_of=arguments.get("as_of"))
    if name == "ao_kg_add":
        return ao_kg_add(subject=arguments["subject"], predicate=arguments["predicate"],
                         object_=arguments["object"], valid_from=arguments.get("valid_from"),
                         source=arguments.get("source"), vault_path=vault_path)
    if name == "ao_palace_rooms":
        return ao_palace_rooms(wing=arguments["wing"], vault_path=vault_path)
    if name == "ao_palace_tunnel":
        return ao_palace_tunnel(wing_a=arguments["wing_a"], wing_b=arguments["wing_b"],
                                room=arguments.get("room", "shared"), vault_path=vault_path)
    if name == "ao_validate":
