...
class Ompa:
    ...
    def __init__(
        ...
        # Explicit type annotation for mypy — these are populated lazily via the property
    ...
    def rebuild_index(self) -> int:
        ...
        semantic = self.semantic  # access once to narrow Optional and avoid double-trigger
        if semantic is None:
            return 0
        semantic.clear()
    ...
    def export_to_shared(
        ...
        if self.dual_config.isolation_mode == IsolationMode.STRICT and confirm:
            # Strict mode: first call returns a preview; caller must confirm before the export writes
