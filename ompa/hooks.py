...
class SessionStartHook(Hook):
    ...
    def execute(self, context: HookContext, **kwargs) -> HookResult:
        try:
            ...
            lines = [
                "## Session Context",
                f"**Date:** {context.timestamp.strftime('%Y-%m-%d (%A)')}",
                "",
            ]
            lines.extend(self._build_north_star_section(vault))
            lines.extend(self._build_git_section(context.vault_path))
            lines.extend(self._build_active_work_section(vault))
            lines.extend(self._build_vault_stats_section(vault))
            lines.extend(self._build_kg_section(context.memory))
            lines.extend(self._build_file_listing_section(vault, context.vault_path))
    ...
    def _build_north_star_section(self, vault: "Vault") -> list[str]:
        """Return lines for the North Star goals section."""
        north_star = vault.get_brain_note("North Star")
        if not north_star:
            return []
        body = self._extract_section(north_star.content, "Current Focus") or north_star.content[:500]
        return ["### North Star (Current Goals)", body, ""]

    def _build_git_section(self, vault_path: Path) -> list[str]:
        """Return lines for the recent git changes section (last 48h)."""
        import shutil
    ...
    def _build_active_work_section(self, vault: "Vault") -> list[str]:
        """Return lines for the active work section."""
        lines = ["### Active Work"]
    ...
    def _build_vault_stats_section(self, vault: "Vault") -> list[str]:
        """Return lines for the vault statistics section."""
    ...
    def _build_kg_section(self, memory: Optional["Ompa"]) -> list[str]:
        """Return lines for the KG section; empty list if no KG data."""
    ...
    def _build_file_listing_section(self, vault: "Vault", vault_path: Path) -> list[str]:
        """Return lines for the truncated vault file listing."""
        lines = ["### Vault Files"]
        all_notes = vault.list_notes()
        for vault_note in sorted(all_notes, key=lambda n: n.path)[:30]:
            try:
                lines.append(f"- {vault_note.path.relative_to(vault_path)}")
            except ValueError:
