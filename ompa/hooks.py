...
class SessionStartHook(Hook):
    ...
    def execute(self, context: HookContext, **kwargs) -> HookResult:
        try:
            ...
            lines.extend(self._file_listing_section(vault, context.vault_path))
    ...
    def _north_star_section(self, vault: Vault) -> list[str]:
    ...
    def _git_changes_section(self, vault_path: Path) -> list[str]:
    ...
    def _kg_stats_section(self, context: HookContext) -> list[str]:
        """Build the knowledge graph stats section (empty if KG unavailable)."""
    ...
    def _file_listing_section(self, vault: Vault, vault_path: Path) -> list[str]:
        """Build the vault file listing section (capped at 30 entries)."""
        lines = ["### Vault Files"]
