...
class SessionStartHook(Hook):
    ...
    def execute(self, context: HookContext, **kwargs) -> HookResult:
        try:
            ...
            self._north_star_section(vault, lines)
            self._git_changes_section(context, lines)
            self._active_work_section(vault, lines)
            self._vault_stats_section(vault, lines)
            self._kg_stats_section(context, lines)
            self._file_listing_section(vault, context, lines)
    ...
    def _north_star_section(self, vault: Vault, lines: list[str]) -> None:
        """Append the North Star goals section."""
    ...
    def _git_changes_section(self, context: HookContext, lines: list[str]) -> None:
        """Append the recent git changes section (last 48 hours)."""
        import shutil
    ...
    def _active_work_section(self, vault: Vault, lines: list[str]) -> None:
        """Append the active work notes section."""
    ...
    def _vault_stats_section(self, vault: Vault, lines: list[str]) -> None:
        """Append the vault statistics section."""
    ...
    def _kg_stats_section(self, context: HookContext, lines: list[str]) -> None:
        """Append the knowledge graph statistics section (if KG has data)."""
    ...
    def _file_listing_section(
        self, vault: Vault, context: HookContext, lines: list[str]
    ) -> None:
        """Append the vault file listing section (capped at 30 entries)."""
    ...
    def _extract_section(self, content: str, section: str) -> str:
        """Extract a named markdown section from content."""
        import re
