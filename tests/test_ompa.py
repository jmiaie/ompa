...
class TestPalace:
    ...
    def test_create_room_idempotent(self):
        """Verify create_room doesn't overwrite existing room data."""
        from ompa import Palace
        with tempfile.TemporaryDirectory() as tmpdir:
            p = Palace(os.path.join(tmpdir, ".palace"))
            p.create_wing("Orion", type="project")
            p.create_room("Orion", "auth")
            p.link_drawer("Orion", "auth", "work/active/auth.md")
            # Re-creating should NOT wipe drawers
            p.create_room("Orion", "auth")
            drawers = p.get_drawers("Orion", "auth")
            assert "work/active/auth.md" in drawers
